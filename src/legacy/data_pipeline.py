...
CSV_LOCAL  = Path(__file__).parent.parent / "data" / "relatorio_cultivares.csv"
...
def _limpar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if COL_CULTIVAR in df.columns:
        # Desmembrar nome secundário da cultivar (separado por '/')
        split_cultivar = df[COL_CULTIVAR].str.split("/", n=1, expand=True)
        df[COL_CULTIVAR] = split_cultivar[0].str.strip()
        if split_cultivar.shape[1] > 1:
            df[COL_NOME_SEC] = split_cultivar[1].str.strip()
            df[COL_NOME_SEC] = df[COL_NOME_SEC].replace("", np.nan)
        else:
            df[COL_NOME_SEC] = pd.NA
