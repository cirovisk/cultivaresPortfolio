...
class InmetExtractor(BaseExtractor):
    ...
    def transform(self, dataframes: dict) -> pd.DataFrame:
        """
        Consolida e agrega dados horários em diários.
        """
        all_dfs = []
    ...
    def run(self, station_ids: list = None) -> pd.DataFrame:
        """
        :param station_ids: Lista de IDs de estação para extrair. Se None, nada será feito (main gerencia).
        """
        if not station_ids:
            self.log.warning("InmetExtractor: Nenhuma estação fornecida para run().")
            return pd.DataFrame()
            
        data = self.extract(station_ids)
